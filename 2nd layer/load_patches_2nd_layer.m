...
% end
f1=load([CIFAR_DIR '/hidstate_1stlayer(poolratio02)_(24f40f6ws9ws12rP20P10Pb01).mat']);
xtr = double([f1.xtr;]);
% xtr = gpuArray(xtr);
...
% xtr = reshape(xtr,[size(xtr,1),size(xtr,2)*size(xtr,3)]);
 ...
 clear f1;
fname = sprintf('hidstate_alloy_pool(ratio02)_2nd_f24f80_ws6ws%d',ws);
...
%                         k=1;
%             sum(sum(cpatch));
%             while sum(cpatch(:))<numel(cpatch)*0.05 % less than 1%               
%                 r = random('unid', CIFAR_DIM(1) - ws + 1);
%                 c = random('unid', CIFAR_DIM(2) - ws + 1);
%                 cpatch = reshape(xtr(mod(i-1,size(xtr,1))+1, :), CIFAR_DIM);
%                 cpatch = cpatch(r:r+ws-1,c:c+ws-1,:);
