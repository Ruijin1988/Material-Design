function [acc, weight, params] = demo_cifar10_2nd_layer...
...
intype = 'binary';
sptype = 'exact';
epsilon = 0.005; %original 0.005, change 0.001 to 0.0001
batchSize = 200; 
if ~exist('split', 'var'),
    split = 'split'; end
...
npatch = 5000;
[xtrain, patches] = load_patches_2nd_layer(npatch, params.rs);
...
if strcmp(params.txtype, 'rot'),
    fname = sprintf('alloy_2nd_hidstate_pool2_(24f80f6ws9ws)_%s_w%d_b%02d_%s_nrot%d_pb%g_pl%g', ...
...
elseif strcmp(params.txtype, 'trans') || strcmp(params.txtype, 'scale'),
    fname = sprintf('alloy_2nd_hidstate_pool2_(24f80f6ws9ws)_%s_w%d_b%02d_%s_ntx%d_gr%d_pb%g_pl%g', ...
...
%     [weight, params] = training_2nd_layer(patches', params, Tlist);
    [weight, params] = training_2nd_layer(patches', params);
end
clear patches;
