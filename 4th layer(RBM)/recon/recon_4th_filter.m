...
% W_Four=gather(weight.vishid);
%pool back 3rd layer filter
% for i = 1:size(W_Four,2)   
%     W_temp=reshape(W_Four(:,i),[36*36 288]);
%     for j = 1:numch
%         W_temp2=reshape(W_temp(:,j),[36 36]);
%         W_temp2=imresize(W_temp2,[36*4 36*4]);
%         W_temp3(:,j)=W_temp2(:);
%     end
%     W_Four_Pool(:,i)=W_temp3(:);
% end
...
for j = 1:size(W_Four,2)
    filter_4th_temp=W_Four(:,j);
...
%     for ii =1:size(filter_t,2)
        ...
        filter_4th = filter_4th_temp((i-1)*size(W_Four,1)/numch+1:i*size(W_Four,1)/numch,:);
        filter_4th=reshape(filter_4th,[sqrt(size(W_Four,1)/numch),sqrt(size(W_Four,1)/numch)]);
