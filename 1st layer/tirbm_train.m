...
if ~isfield(params, 'fname'),
    ...
    elseif strcmp(params.txtype,'trans') || strcmp(params.txtype,'scale'),
        fname = sprintf('WB_%s_w%d_b%02d_%s_ntx%d_gr%d_pb%g_pl%g', ...
...
% momentum
initialmomentum  = 0.1; % change from 0.1 to 0.01
finalmomentum    = 0.9;
...
for t = 1:params.maxiter,
    ...
    for i = 1:nbatch,
        ...
        % reshape for speedup
%         weight.hidbias=zeros(2,4);
%         weight.visbias=zeros(144,1);
        ...
        if mod(i, 5) == 0, % changed from 5 to 20, 10/28/2015
